...
async fn router_health_and_openapi_are_public() {
    ...
    assert!(json["paths"]["/api/v1/{provider_id}/register"].is_object());
    assert!(json["paths"]["/api/v1/{provider_id}/register"]["post"]
        .get("security")
        .is_none());
...
fn register_body(provider_id: &str, hardware_private: &[u8; 32]) -> String {
    let (public, signature) =
        relay::sign_register_proof(hardware_private, provider_id).expect("sign");
    serde_json::json!({
...
async fn register_requires_identity_and_matching_provider() {
    ...
    let missing = app
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/acme/register")
                .header("Content-Type", "application/json")
                .body(Body::from(register_body("acme", &hw_sk)))
                .unwrap(),
        )
        .await
        .unwrap();
...
async fn register_mints_unauthenticated_key_signed_by_keyquorum() {
    ...
    let wrong = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/other/register")
                .header("Content-Type", "application/json")
                .body(Body::from(register_body(provider_id, &hw_sk)))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    assert!(token.starts_with("kq_"));
    assert_eq!(json["provider_id"], provider_id);
    assert_eq!(json["hardware_fingerprint"], keys::fingerprint(&hw_pk));

    let signature: [u8; 64] = hex::decode(json["keyquorum_signature"].as_str().expect("sig"))
        .expect("hex")
        .try_into()
        .expect("64");
    let key_hash = relay::hash_bearer(token).expect("hash");
    crate::signing::verify_signature(
        &issued.relay_public,
        &relay::receipt_preimage(
            provider_id,
            &hw_pk,
            json["hardware_fingerprint"].as_str().expect("fp"),
            json["id"].as_i64().expect("id"),
            &key_hash,
            json["registered_at"].as_str().expect("when"),
        )
        .expect("preimage"),
        &signature,
    )
    .expect("keyquorum receipt");
    ...
    let pushed = app
        .oneshot(
            Request::builder()
                ...
                .header("Content-Type", "application/octet-stream")
                .body(Body::from(sample_envelope().0))
                .unwrap(),
        )
        .await
        .unwrap();
