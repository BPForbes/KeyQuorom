...
issuer); HTTP cannot create or rotate bearers. `kq-relay` is **provider-only
setup** (serve the mailbox and mint keys). Do not document it in README or
other customer-facing docs — buyers get a URL and an API key and use
`keyquorum loadkey` / `relay push` / `relay pull`. Default `cargo build`
produces `keyquorum` only; build the host with `--features provider`.
`keyquorum loadkey` calls `POST /keycheck` (no auth) and stores that hash plus
a sealed bearer in the personal SQLite file. Later commands re-check the hash
and inject the bearer. Never commit bearers, `.kqpb` files, or the relay
database.

## Setup / build / test

- Build (customer CLI): `cargo build --release` (produces `keyquorum` only)
- Build (provider mailbox): `cargo build --release --features provider`
