...
- **Notification:** Phase 1 writes per-recipient `.kqpb` packages (sealed to that person's encryption pub) plus local `bridge_events` and `.kqbn` eviction notices. Phase 2 (later) is a richer signed update bundle for operators with no shared live DB.
...
## Phase 2 (deferred — not this change)

- Signed bridge-update export/import so `M.S` can push membership + `public_key` + `salt` updates to `M.A` without a shared live DB.
- Mention only as a short README “later” note.
